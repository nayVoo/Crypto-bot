...
#Сюда из другой функции кидаю монеты и они сортируються, потом сделать так что бы ещё и delete_lsit кидался в дальнейшем
def sort_token(to_sort):
	to_sort = [x for x in token_list if x not in delete_list]
	return to_sort
